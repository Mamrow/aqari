...
Environment: copy `.env.example` to `.env` and set `EXPO_PUBLIC_SUPABASE_URL` / `EXPO_PUBLIC_SUPABASE_ANON_KEY` (from the Supabase project's Settings → API) and `GOOGLE_MAPS_API_KEY_ANDROID`. `app.config.js` wraps `app.json` to inject the Maps key into `android.config.googleMaps.apiKey` at build time.

...
**Auth is phone + password on Supabase's native phone provider, with a one-time code.** Sign-up takes name/phone/password and sends a 6-digit code (`supabase.auth.signUp({ phone, password, options: { channel } })`), which `verifySignUpOtp` redeems before the `profiles` row is written — abandoning the code screen leaves no profile behind. Day-to-day sign-in is `signInWithPassword({ phone, password })` with no code at all, which is what keeps the per-message cost off the common path. "Forgot password" is `signInWithOtp({ shouldCreateUser: false })` — that flag is load-bearing, without it a typo'd number silently becomes a new account — then `verifyOtp` + `updateUser({ password })`; redeeming the code *is* the proof of ownership, so there's no admin API, no emailed link, and no service-role key anywhere in the flow. `src/utils/otp.js`'s `OTP_CHANNEL` is the single switch between `'sms'` and `'whatsapp'`; note `verifyOtp` takes `type: 'sms'` either way, since the channel only affects delivery. Twilio credentials live in the Supabase dashboard, never in the app — there is no Twilio SDK in the bundle. `docs/phone-auth-setup.md` covers the dashboard config, the trial's verified-numbers limit, and why WhatsApp needs Meta approval before that constant can flip.

**No email address is collected anywhere**, and that's a constraint to preserve, not an accident: the App Store privacy label declares Email Address as not collected. The previous design registered each phone as a synthetic `@phone.aqari.dev` address and mailed reset links via Resend through a `send-password-reset` Edge Function — all of that (function, `profiles.email`, `get_reset_email_for_phone`, `private.password_reset_attempts`, the `aqari://reset-password` deep-link handling) is deleted; see `supabase/migration_phone_auth_no_email.sql`. Session storage (`src/lib/supabase.js`) is still a custom `LargeSecureStore`: a random AES key lives in `expo-secure-store` (small, but real Keychain/Keystore), the session ciphertext in `AsyncStorage` (unbounded size, but not secure storage) — plaintext tokens never touch disk.
